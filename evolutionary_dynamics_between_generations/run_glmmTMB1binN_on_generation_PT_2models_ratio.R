...
normalizedCounts <- read.table("Galaxy371-Normalized_counts.tabular", sep = '\t', header=TRUE, stringsAsFactors = TRUE)
...
for(i in 1:ncol(data1)) {       # for-loop over columns
  ...
  meanWPTG9 <- (data3[8,1]+data3[9,1]+data3[12,1])/3    #PT_TREATMENT_counts are the  populations w from G9 in lines 8,9 and 12 of the first column
  meanCPTG9 <- (data3[7,1]+data3[10,1]+data3[11,1])/3       #PT_CONTROL_counts are the populations C from G9 and are in lines 7,10 and 11 of the first column
  upOrDownG9<-meanWPTG9/meanCPTG9
  ...
  meanWPTG23 <- (data3[4,1]+data3[5,1]+data3[6,1])/3    #WPT_TREATMENT_counts are in lines 7,8 and 9 of the first column
  meanCPTG23 <- (data3[1,1]+data3[2,1]+data3[3,1])/3       #WPT_CONTROL_counts are in lines 4,5 and 6 of the first column
  upOrDownG23<-meanWPTG23/meanCPTG23
  ...
  try(write.table(transposed_of_AnovaGlmmTMB1_binN, "2factors_low_latitude.csv" , append = TRUE, sep = '\t', col.names = FALSE, row.names = TRUE))
