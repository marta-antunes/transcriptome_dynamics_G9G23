...
normalizedCounts <- read.table("Galaxy96-Normalized_counts.tabular", sep = '\t', header=TRUE, stringsAsFactors = TRUE, dec=".")
...
for(i in 1:ncol(data1)) {       # for-loop over columns
  ...
  try(write.table(transposed_of_AnovaGlmmTMB1_binN, "PT_SelectionAtG9.csv" , append = TRUE, sep = '\t', col.names = FALSE, row.names = TRUE))
