...
normalizedCounts <- read.table("Galaxy194-Normalized_counts.tabular", sep = '\t', header=TRUE, stringsAsFactors = TRUE, dec=".")
...
for(i in 1:ncol(data1)) {       # for-loop over columns
  ...
  try(write.table(transposed_of_AnovaGlmmTMB1_binN, "NL_SelectionAtG9.csv" , append = TRUE, sep = '\t', col.names = FALSE, row.names = TRUE))
